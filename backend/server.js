...
//middleware that checks if JWT token exists and verifies it
...
app.use(function (req, res, next) {
  ...
  jwt.verify(token, process.env.JWT_SECRET, function (err, user) {
    if (err) {
      ...
      req.user = user; //set the user to request so other routes can use it
      req.token = token;
      next();
    }
  });
